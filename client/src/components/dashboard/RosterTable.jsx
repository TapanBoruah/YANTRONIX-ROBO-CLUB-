...
const RosterTable = ({ roster, loggedInUser, onEdit }) => {
  return (
    <div className="overflow-x-auto w-full scrollbar-thin scrollbar-thumb-cyber-border">
          ...
          {roster.map((m) => (
            <tr key={m.id} className="hover:bg-cyber-darker/30 transition-colors">
              ...
              {(loggedInUser?.role === 'super' || loggedInUser?.role === 'rep') && (
                <td className="px-6 py-4 font-mono text-xs text-cyan-400 select-all whitespace-nowrap">
